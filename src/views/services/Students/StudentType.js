...
const StudentType = () => {
  ...
  const AddModalOpen = (id) => {
    if (id !== "add") {
      get(`StudentType/Get/${id}`).then((res) => {
        setInfo(res);
      });
    } else {
      setInfo({});
  ...
  const handleSubmit = (e) => {
    ...
    if (!info?.id) {
      ...
      put("StudentType/Update", subdata).then((res) => {
  ...
  const handleUpdateSubmit = () => {
    ...
    dispatch(
      UpdateDepartment(subData, (action) => {
        ...
        addToast(action, {
          appearance:
            action == "Department updated successfully." ? "success" : "error",
          autoDismiss: true,
        });
        setdepartment("");
        localStorage.removeItem("depName");
  ...
  const handleDelete = (id) => {
    remove(`StudentType/Delete/${id}`).then((res) => {
      addToast(res, {
        appearance: "error",
        autoDismiss: true,
      });

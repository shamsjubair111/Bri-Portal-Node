...
const ExamTestTypeAttribute = () => {
  ...
  const examTestTypeAttributeId = localStorage.getItem(
    "examTestTypeAttributeId"
  );
  ...
  const handleSubmit = (e) => {
    ...
    post("ExamTestTypeAttribute/Create", subData).then((res) => {
      addToast(res?.data?.message, {
        appearance: "error",
        autoDismiss: true,
      });
      get(
        `ExamTestTypeAttribute/GetByExamTestType/${localStorage.getItem(
          "examTestTypeAttributeId"
        )}`
      ).then((res) => {
        setExamTestTypeAttribute(res);
  ...
  const handleDelete = (data) => {
    remove(`ExamTestTypeAttribute/Delete/${data?.id}`).then((res) => {
      setDeleteModal(false);
      addToast(res, {
        appearance: "error",
        autoDismiss: true,
        ...
        <CardHeader className="page-header">
          ...
          <div className="page-header-back-to-home">
            <span className="text-white" onClick={backToList}>
              {" "}
              <i className="fas fa-arrow-circle-left"></i> Back to Exam Test
              Type List
            </span>
          </div>
        </CardHeader>
            ...
            <Col md="8">
              <div className="table-responsive page-header ">
                <Table className="table-sm table-bordered">
                  ...
                  <tbody>
                    {examTestTypeAttribute?.map((exam, i) => (
                      <tr key={exam?.id}>
                        <td>{exam?.name}</td>
                        ...
                        <td>
                          <Button
                            className="mx-2"
                            onClick={() => toggleDanger(exam?.name, exam?.id)}
                            color="danger"
                          >

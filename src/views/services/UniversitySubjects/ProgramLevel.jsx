...
const ProgramLevel = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (programLvlId != undefined) {
      ...
      setButtonStatus(true);
      setProgress1(true);
      const returnValue = post(`ProgramLevel/Create`, subData).then(
        (action) => {
          setButtonStatus(false);
          setProgress1(false);
          setSuccess(!success);
        ...
        Submit
              ...
              <div className="table-responsive">
                <Table className="table-sm table-bordered">
                  ...
                  <tbody>
                    {programLevelList?.map((program, i) => (
                      <tr key={program.id} style={{ textAlign: "center" }}>
                        <th scope="row">{i + 1}</th>
                        <td>{program?.name}</td>
                        ...
                        <td>
                          <ButtonGroup>
                            ...
                            {permissions?.includes(
                              permissionList?.Update_program_level_info
                            ) ? (
                              <ButtonForFunction
                            ...
                            {permissions?.includes(
                              permissionList?.Delete_program_level
                            ) ? (
                              <ButtonForFunction
                                className={"mx-1 btn-sm"}
                                func={() =>
                                  toggleDanger(program?.name, program?.id)
                                }
                                color={"danger"}
                          ...
                          >
                            <ModalBody>
                              <p>
                                Are You Sure to Delete this <b>{ProgramName}</b>{" "}

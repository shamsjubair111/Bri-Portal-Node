...
import { Card, CardHeader, CardBody, Table, ButtonGroup, Modal, ModalHeader,ModalBody, ModalFooter, Button, Form, FormGroup, Col, Input } from 'reactstrap';
...
const AddCity = () => {
  ...
  const handleDeleteUniState = () =>{
    ...
    remove(`UniversityCity/Delete/${delData?.id}`)
    .then(res => {
        addToast(res,{
            appearance:'error',
            autoDismiss: true
  ...
  const handleSubmit =  (event) => {
    ...
    if(uniValue == 0){
        ...
        if(!data?.id){
            ...
            setProgress(true);
            post(`UniversityCity/Create`,subData)
            .then(res => {
            ...
            setButtonStatus(true);
            setProgress(true);
            put(`UniversityCity/Update`,subData)
            .then(res => {
                setButtonStatus(false);
                setProgress(false);
                if(res?.status == 200 && res?.data?.isSuccess == true){
                  ...
                  <FormGroup row className="has-icon-left position-relative">
                    ...
                    <Col md="8">
                      <Input
                        ...
                        onChange={(e) => {setCityVar(e.target.value);
                        }}
                        />

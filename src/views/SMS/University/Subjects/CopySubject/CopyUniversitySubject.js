...
const CopyUniversitySubject = () => {
    ...
    useEffect(()=>{
      ...
      if(newSubId != undefined){
        get(`Subject/Get/${newSubId}`).then(res=> {
            setSubject(res);
            setSubId(res?.id);
            setSubName(res?.name);
...
//   }
  ...
  const toggle = (tab) => {
    ...
    if(newSubId != undefined){
      ...
      if (tab == "3") {
        history.push(`/copyAndAddUniversitySubjectDeliveryPattern/${id}/${subjId}/${newSubId}`);
      }
      if (tab == "4") {
        history.push(`/copyAndAddUniversitySubjectRequirements/${id}/${subjId}/${newSubId}`);
      }
      if (tab == "5") {
        history.push(`/copyAndAddUniversitySubjectDocumentRequirement/${id}/${subjId}/${newSubId}`);
      }
    }
    else{
      if (tab == "2") {
        history.push(`/copyAndAddUniversitySubjectFee/${id}/${subjId}/${subjectId}`);
      }
      if (tab == "3") {
        history.push(`/copyandAddUniversitySubjectDeliveryPattern/${id}/${subjId}/${subjectId}`);
      }
      if (tab == "4") {
        history.push(`/copyAndAddUniversitySubjectRequirements/${id}/${subjId}/${subjectId}`);
      }
      if (tab == "5") {
        history.push(`/copyAndAddUniversitySubjectDocumentRequirement/${id}/${subjId}/${subjectId}`);
      }
    }
  };

...
const AddUniversityApplicationDocument = () => {
    ...
    useEffect(()=>{
      ...
      get(`UniversityApplicationDocument/GetByUniversity/${localStorage.getItem('id')}`)
      .then(res=>{
  ...
  const handleSubmit = e =>{
    ...
    subData.append('documentFile',FileList1.length == 0 ? null : FileList1[0]?.originFileObj);
    ...
    if(documentValue == 0){
      ...
      .then(res => {
        ...
        put(`UniversityApplicationDocument/Update`, subData)
    .then(res => {
  ...
  const handleDeletePermission = (id) => {
    ...
    const returnValue = remove(`UniversityApplicationDocument/Delete/${id}`).then((action) => {
...

        {
          ...
          <thead className="thead-uapp-bg">
            ...
            <tbody>
              ...
              {
                applicationList?.map((application, i) => <tr key={application?.id} style={{ textAlign: "center" }}>
                  <th scope="row">{i + 1}</th>
                  <td>{application?.name}</td>
                  <td>{application?.description}</td>
                  <td>{application?.documentRequirementStatus?.name}</td>
                  <td><a href={rootUrl+application?.document?.fileUrl} target="_blank" download>Download</a></td>
                  <td>
                    ...
                    <Modal isOpen={deleteModal} toggle={() => setDeleteModal(!deleteModal)} className="uapp-modal">
                      ...
                      <ModalBody>
                        <p>Are You Sure to Delete this <b>{localStorage.getItem('applicationName')}</b> ? Once Deleted it can't be Undone!</p>
                      </ModalBody>

                      <ModalFooter>
                        <Button color="danger" onClick={() => handleDeletePermission(localStorage.getItem('applicationId'))}>YES</Button>
          ...
          <Form onSubmit={handleSubmit} className="mt-5">
                ...
                <FormGroup row className="has-icon-left position-relative">
                  ...
                  {
                    selectedId !== 0 ? <Input type="hidden" id="Id" name="Id" value={selectedId} />
                    :
                    null
                  }
                </FormGroup>
                ...
                <div className = 'row'>
                    ...
                    <div className='col-md-3'>
                      ...
                      src={rootUrl+applicationObject?.document?.fileUrl}
                    />
...

                {
                     ...
                     <>
                      ...
                      <div>
                        {
                            selectedId !== 0 || applicationList.length>0?
                            // <Button onClick={cancel} color="danger uapp-form-button float-right">Cancel</Button>
